...
def tile_data(scan_pos, args):
    ...
    try:    
        ...
        mon = glob.glob(os.path.join(base, scan, '??????_??????.mon.rxp'))[0]
        sp = int(scan.replace(args.prefix, ''))
        
        if args.verbose:
            with args.Lock:
                print('rxp -> xyz:', mon if args.test else rxp)
        
        fn_matrix = glob.glob(os.path.join(base, 'matrix', f'{scan}.*'))[0]
        matrix = np.loadtxt(fn_matrix)
        st_matrix = ' '.join(matrix.flatten().astype(str))
    
        # pdal commands as dictionaries
        read_in = {"type":"readers.rxp",
                   "filename":mon if args.test else rxp,
                   "sync_to_pps": "false",
        ...
        cmds = [read_in, dev_filter, transform, tile]
        ...
        for arr in pipeline.arrays:
            ...
            arr.loc[:, 'sp'] = sp
            arr = arr[['x', 'y', 'z', 'refl', 'dev', 'sp']] # save only relevant fields
...
def xyz2ply(xyz_path, args):
    ...
    tmp = pd.DataFrame(np.fromfile(open_file, dtype='float64,float64,float64,float32,float32,int64'))
    tmp.columns = ['x', 'y', 'z', 'refl', 'dev', 'sp']
    ply_io.write_ply(xyz_path.replace('.xyz', '.ply'), tmp[['x', 'y', 'z', 'refl', 'dev', 'sp']])
    os.unlink(xyz_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--deviation', type=float, default=15, help='buffer plot extraction')
    ...
    args.bbox = np.array([matrix_arr.min(axis=0)[:2], 
                          matrix_arr.max(axis=0)[:2] + args.tile]).T.flatten() // args.tile * args.tile
    if args.verbose: print('bounding box:', args.bbox)

    # create tile db
    X, Y = np.meshgrid(np.arange(args.bbox[0], args.bbox[1], args.tile), 
                       np.arange(args.bbox[2], args.bbox[3], args.tile))
